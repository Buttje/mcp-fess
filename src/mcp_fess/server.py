...
def _mime_type_for_image(image_path: Path) -> str:
    ...
    if mime and mime.startswith("image/"):
        return mime
    # Fallback based on common image extensions used by the snippet engine
...
3. Call `fetch_content_chunk` (preferred) or `fetch_content_by_id` to read extracted UTF-8 text evidence from the index.
   - Text content may contain `<IMAGE: /absolute/path/to/image.png>` markers for extracted images.
   - To retrieve an image, use the `fess_get_image` tool with `image_path` set to the absolute path, or read the `fess:///image/{filename}` resource where `{filename}` is the basename of the path.
...
5. Refine the query using evidence; optionally use `suggest` and `popular_words` to expand/pivot."""
    ...
    def _descriptor_text_source(self) -> str:
        ...
        return (
            ...
            "Retrieve images via the `fess_get_image` tool (pass `image_path`) "
            "or by reading the `fess:///image/{filename}` resource (use the basename of the path)."
        )
...
Snippets may contain `<IMAGE: /absolute/path>` markers for images extracted from the source document;
use the `fess_get_image` tool (pass `image_path`) or read `fess:///image/{{filename}}` resource to retrieve them.
...
Use this to locate the source file on the local filesystem, for example to open it
directly or to resolve relative image paths found in the document content.
...
Returns:
    ...
    - 'original_path': Absolute filesystem path of the original document"""
        ...
        @self.mcp.tool(name="fess_get_image")
        async def get_image(image_path: str) -> str:
            return await self._handle_get_image({"imagePath": image_path})

        get_image.__doc__ = """Retrieve an image extracted from a source document.

Use this tool when document content contains an `<IMAGE: /absolute/path/to/image.png>` marker.
Pass the absolute path from the marker as `image_path` to obtain the image as base64-encoded data.

Alternatively, you can read the image via the MCP resource `fess:///image/{filename}` where
`{filename}` is the basename (e.g. `abc123_p1_i0_def456.png`) of the path in the marker.

Args:
    image_path: Absolute filesystem path of the image file (from the `<IMAGE: ...>` marker)

Returns:
    JSON with:
    - 'data': Base64-encoded image bytes
    - 'mimeType': MIME type of the image (e.g. 'image/png' or 'image/jpeg')
...
Images are extracted and saved as files; the snippet text contains `<IMAGE: /absolute/path>`
...
{self._descriptor_limits()}"""
        ...
        @self.mcp.resource("fess:///image/{image_id}")
        async def read_image(image_id: str) -> ResourceResult:
            try:
                image_path = self._resolve_image_path(image_id)
                image_bytes = image_path.read_bytes()
                mime_type = _mime_type_for_image(image_path)
                return ResourceResult([ResourceContent(image_bytes, mime_type=mime_type)])
            except Exception as e:
                logger.error(f"Failed to read image resource '{image_id}': {e}")
                raise

        read_image.__doc__ = """Image file extracted from a source document.

The `image_id` is the filename (basename) of the image, e.g. `abc123_p1_i0_def456.png`.
This filename appears at the end of every `<IMAGE: /absolute/path/to/image_id>` marker in
snippet content.

Returns the raw image bytes with the appropriate MIME type (image/png or image/jpeg)."""
    ...
    def _resolve_image_path(self, image_id: str) -> Path:
        ...
        Searches under the host Fess data directory (if configured) or falls back
        to treating ``image_id`` as an absolute path when it starts with '/'.

        Args:
            image_id: Filename of the image (e.g. ``abc123_p1_i0_def456.png``).
        ...
        Raises:
            ...
            ValueError: If ``image_id`` contains path traversal sequences.
        """
        # Basic security check - reject any path separators or traversal sequences
        if "/" in image_id or "\\" in image_id or image_id.startswith("."):
            raise ValueError(f"Invalid image_id '{image_id}': must be a plain filename")

        if self.config.fessComposePath:
            try:
                ...
                for candidate in host_data_dir.rglob(image_id):
                    if candidate.is_file():
                        return candidate
            except Exception as e:
                logger.debug(f"Image lookup via compose path failed: {e}")

        raise FileNotFoundError(
            f"Image '{image_id}' not found. "
            "Ensure snippets have been generated with fess_generate_snippets."
        )

    async def _handle_get_image(self, arguments: dict[str, Any]) -> str:
        """Handle getImage tool call - return image as base64-encoded JSON."""
        image_path_str = arguments.get("imagePath", "")
        if not image_path_str:
            return json.dumps({"error": "imagePath is required"})

        image_path = Path(image_path_str)

        # Security: reject non-absolute paths
        if not image_path.is_absolute():
            return json.dumps({"error": "imagePath must be an absolute filesystem path"})
        try:
            # strict=True raises FileNotFoundError for missing paths, avoiding
            # a TOCTOU window between resolution and the existence check below.
            image_path = image_path.resolve(strict=True)
        except FileNotFoundError:
            return json.dumps({"error": f"Image not found: {image_path_str}"})
        except Exception as e:
            return json.dumps({"error": f"Cannot resolve path: {e}"})
